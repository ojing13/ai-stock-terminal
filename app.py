...
- [시장 우려 및 리스크 직시]: 특정 지표가 악화되었거나 주가가 하락 중이라면, 뉴스 맥락을 파악해 실제 시장이 무엇을 두려워하는지(예: 실적 둔화, 부채 증가로 인한 이자 부담 등) 평가에 명확히 반영하세요. 무조건적인 장밋빛 전망은 배제하세요.
- [기계적 해석 금지]: PBR, PER 등의 수치가 비정상적(음수, 과도한 고평가 등)일 때 교과서적인 잣대를 기계적으로 들이대지 마세요. 시장의 실질적인 맥락을 먼저 유추하여 평가하세요.
- [뉴스 직접 인용 절대 금지]: 제공된 뉴스 동향에서는 '현재 기업이 처한 업황이나 핵심 모멘텀'에 대한 정보만 추출하여 분석의 근거로 쓰세요. 작성 시 "최신 뉴스에 따르면~", "기사 제목에서 알 수 있듯이~" 같은 촌스러운 표현이나 기사 제목 자체를 절대 적지 마세요.
