...
<style>
    ...
    * {
    ...
    /* === 모바일에서도 언어 선택기가 우측 상단에 예쁘게 고정되도록 설정 === */
    div[data-testid="stHorizontalBlock"]:first-of-type {
        flex-wrap: nowrap !important;
        align-items: center !important;
    }
    div[data-testid="stHorizontalBlock"]:first-of-type [data-testid="column"]:nth-child(2) {
        flex-grow: 0 !important;
        width: 90px !important;
        min-width: 90px !important;
    }
    div[data-testid="stHorizontalBlock"]:first-of-type div[data-baseweb="select"] {
...
with col_lang:
    lang = st.selectbox("Language", ["🇰🇷 KR", "🇺🇸 EN", "🇯🇵 JP"], label_visibility="collapsed")
...
with col_title:
    if lang == "🇰🇷 KR":
        ...
        sys_lang = "English"
        st.title("AI Stock Analysis Terminal")
    else:
        sys_lang = "日本語"
        st.title("AI 株式分析ターミナル")
st.markdown("---")

col_search, _ = st.columns([1, 2])
with col_search:
    user_input = st.text_input("분석할 종목명 또는 티커 (예: 삼성전자, AAPL, 7203)", "")
...
if user_input:
    ...
    if not hist_basic.empty:
        ...
        # 뉴스 기사 수집량 100개 및 일본 뉴스 연동
        try:
            if is_korean_stock:
                rss_url = f"https://news.google.com/rss/search?q={user_input}+주식&hl=ko-KR&gl=KR&ceid=KR:ko"
            elif is_jp_stock:
                rss_url = f"https://news.google.com/rss/search?q={ticker.replace('.T', '')}+株&hl=ja&gl=JP&ceid=JP:ja"
            else:
        ...
        with tab1:
            ...
            if st.button("AI 차트 추세 분석 실행"):
                with st.spinner("순수 기술적 관점에서 차트를 분석하는 중입니다..."):
                    ...
                    3. [이동평균선 표기 규칙]: 이동평균선을 언급할 때 숫자와 단위 사이에 하이픈(-)을 절대 넣지 마세요.
                    ...
                    7. [어조 설정]: 정중한 전문가 톤을 유지하세요. 반말은 절대 금지합니다.
                    8. [항목 제한]: 분석 항목은 무조건 '1. 단기적인 추세', '2. 장기적인 추세' 딱 두 가지만 출력하세요.
                    9. [뉴스 및 기사 수 언급 절대 금지]: 당신은 100개의 최신 시장 동향 기사를 배경지식으로 제공받았지만, 출력물에 기사에 대한 언급을 절대 하지 마세요. 오직 차트와 가격 움직임을 바탕으로 하되, 배경지식을 활용해 틀린 분석(환각)을 하지 않는 용도로만 조용히 참고하세요.
                    ...
                    🚨 [출력 언어 지시사항]: 당신의 모든 분석 결과는 반드시 **{sys_lang}**로 번역하여 출력하세요!
                    """
                    try:
...
- [뉴스 및 기사 수 언급 절대 금지]: "제공된 데이터에 따르면", "수집된 기사/뉴스에서" 등의 표현을 완벽하게 금지합니다. '뉴스', '기사'라는 단어 자체를 출력문에 쓰지 마세요.
        ...
        with tab4:
            ...
            if st.button("원클릭 종합 분석 리포트 생성"):
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    - [핵심 강조]: 전체 리포트에서 핵심이 되는 주요 단어나 결과 문장은 반드시 **굵은 글씨(**)**로 강조해서 핵심을 짚어주세요.
                    - [직접 인용 및 작위적 표현 완벽 금지]: 리포트 내에 '뉴스', '기사'라는 단어를 아예 사용하지 마세요. 마치 당신이 현업에서 직접 시장을 모니터링하며 얻은 팩트인 것처럼 유려하게 서술하세요.
                    - [배경 지식 총동원]: 당신이 학습한 해당 기업의 최근 거시경제 환경, 산업 트렌드, 경쟁사 동향 등을 융합하여 인과관계를 설명하세요.
                    - 마크다운 렌더링 오류를 막기 위해 절대 물결표 및 달러 기호를 사용하지 마세요. (금액은 반드시 '{currency}'으로 표기할 것)
                    - [기사 수 언급 절대 금지]: 수집된 기사의 개수나 규모를 직접적으로 절대 언급하지 마세요.
