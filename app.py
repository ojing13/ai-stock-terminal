...
<style>
    ...
    * {
    ...
    /* === Plotly 모바일 스크롤 및 터치 문제 해결 === */
    .js-plotly-plot .plotly, .js-plotly-plot .plotly div {
        touch-action: pan-y !important; /* 모바일에서 위아래 스크롤을 허용 */
    }
</style>
    ...
    if not hist_basic.empty:
        ...
        with tab1:
            ...
            if st.button("AI 차트 추세 분석 실행"):
                with st.spinner("일봉, 주봉, 월봉 데이터를 모두 종합하여 순수 기술적 관점에서 차트를 분석하는 중입니다..."):
                    
                    # 일봉, 주봉, 월봉 데이터를 모두 불러와서 처리하는 내부 함수
                    def get_formatted_history(interval_str, ma_config):
                        ...
                        cols_to_export = ['Close'] + [f'MA_{w}' for w, _, _ in ma_config]
                    ...
                    prompt = f"""종목 {ticker}의 일봉, 주봉, 월봉 전체 가격 및 이동평균선(MA) 데이터입니다.
                    ...
                    [🚨 기술적 분석 핵심 지시사항 🚨]
                    1. [다중 주기 종합 분석 및 필터링]: 일봉, 주봉, 월봉 데이터를 모두 종합적으로 고려하여 큰 흐름과 단기 흐름을 유기적으로 파악하세요. 단, 이동평균선 수치에만 과도하게 매몰되지 말고 전체적인 가격 흐름(Price Action), 지지와 저항, 캔들의 방향성과 추세의 강도 등을 최우선으로 분석하세요. 설명에 꼭 필요한 핵심 단서만 골라서 자연스럽게 제시하고 불필요한 정보는 과감히 생략하세요.
                    2. [이동평균선 표기 규칙]: 이동평균선을 언급할 때 '13-주 이동평균선'처럼 숫자와 단위 사이에 하이픈(-)을 절대 넣지 마세요. 반드시 '13주 이동평균선', '20일 이동평균선', '9개월 이동평균선'과 같이 올바르고 자연스러운 한국어로 띄어쓰기 또는 붙여쓰기 하세요.
                    3. [초보적 서술 금지]: 단순한 가격 등락 나열("얼마에서 얼마로 올랐다")은 강력히 금지합니다.
                    4. 마크다운 수식 오류 방지: 가격 범위나 기간 표시 시 절대 물결표 및 달러 기호를 사용하지 마세요. (금액은 반드시 '{currency}'로 표기할 것)
                    5. 기계적인 기간 설정 금지: 장기 추세를 분석할 때 스스로 의미 있는 기간을 정의하세요.
                    6. [가독성 철저]: 글머리 기호(-, *, • 등 땡땡 표시)를 절대 사용하지 마세요. 소제목은 마크다운 헤딩(###)으로 작성하고, 문단과 문단 사이에는 빈 줄(Enter 2번)을 넣어 완벽하게 분리하세요.
                    7. [핵심 강조]: 분석 내용 중 핵심이 되는 중요한 단어나 문장 및 주요 가격은 반드시 **굵은 글씨(**)**로 강조해서 한눈에 들어오게 하세요. 단, 폰트 크기나 색상은 절대 변경하지 마세요.
                    8. [어조 설정]: 반드시 '~습니다', '~입니다' 형태의 정중체를 사용하세요. 반말은 절대 금지하며, 지나치게 깍듯한 극존칭은 피하고 깔끔하고 객관적인 톤을 유지하세요.
                    9. [항목 제한]: 분석 항목은 무조건 '1. 단기적인 추세', '2. 장기적인 추세' 딱 두 가지만 출력하세요. '중기적인 추세' 같은 임의의 항목을 절대 추가하지 마세요.

                    [출력 형식 가이드]
                    ### 1. 단기적인 추세 (Short-term trend)

                    단기적인 가격 흐름과 매수/매도 모멘텀을 분석합니다. 유의미할 경우에 한해 주요 지지/저항 가격이나 이동평균선 등의 지표를 근거로 자연스럽게 제시하세요. 글머리 기호 없이 일반 문단으로 작성하세요.

                    ### 2. 장기적인 추세 (Long-term trend)

                    일/주/월봉을 아우르는 큰 흐름에서의 추세와 차트 구조를 분석합니다. 유의미할 경우에 한해 중장기 이동평균선 배열이나 거시적 가격대 돌파 여부를 언급하세요. 글머리 기호 없이 일반 문단으로 작성하세요.
                    """
