                ...
                with st.spinner("순수 기술적 관점에서 차트를 분석하는 중입니다..."):
                    ...
                    for w, name, color in ma_settings:
                        if f'MA_{w}' in filtered_history.columns:
                            cols_to_export.append(f'MA_{w}')
                    
                    df_ta = filtered_history[cols_to_export].copy()
                    df_ta.index = df_ta.index.strftime('%Y-%m-%d')
                    df_ta = df_ta.round(2)
                    full_data_csv = df_ta.to_csv(header=True)
                    
                    prompt = f"""종목 {ticker}의 전체 가격 및 이동평균선(MA) 데이터입니다. (차트 주기: {interval_option})
                    ...
                    [🚨 기술적 분석 핵심 지시사항 🚨]
                    1. [유연한 지표 및 가격 활용]: 차트를 분석하여 **실제로 유의미하고 중요한 경우에만** 구체적인 지지/저항선 가격대(원/달러)나 기술적 지표(이동평균선 배열, 이격도, 골든/데드크로스 등)를 언급하세요. 무조건 기계적으로 모든 지표나 가격을 나열할 필요는 없으며, 추세 설명에 꼭 필요한 맥락에서만 자연스럽게 가격과 기술적 근거를 조화롭게 설명하세요.
                    2. [초보적 서술 금지]: 단순한 가격 등락 나열("얼마에서 얼마로 올랐다")은 강력히 금지합니다.
                    3. 마크다운 수식 오류 방지: 가격 범위나 기간 표시 시 절대 물결표 및 달러 기호를 사용하지 마세요. (금액은 반드시 '{currency}'로 표기할 것)
                    4. 기계적인 기간 설정 금지: 장기 추세를 분석할 때 스스로 의미 있는 기간을 정의하세요.
                    5. [가독성 철저]: 글머리 기호(-, *, • 등 땡땡 표시)를 절대 사용하지 마세요. 소제목은 마크다운 헤딩(###)으로 작성하고, 문단과 문단 사이에는 빈 줄(Enter 2번)을 넣어 완벽하게 분리하세요.
                    6. [핵심 강조]: 분석 내용 중 핵심이 되는 중요한 단어나 문장 및 주요 가격은 반드시 **굵은 글씨(**)**로 강조해서 한눈에 들어오게 하세요. 단, 폰트 크기나 색상은 절대 변경하지 마세요.
                    7. [어조 설정]: 반드시 '~습니다', '~입니다' 형태의 정중체를 사용하세요. 반말은 절대 금지하며, 지나치게 깍듯한 극존칭은 피하고 깔끔하고 객관적인 톤을 유지하세요.
                    8. [항목 제한]: 분석 항목은 무조건 '1. 단기적인 추세', '2. 장기적인 추세' 딱 두 가지만 출력하세요. '중기적인 추세' 같은 임의의 항목을 절대 추가하지 마세요.
