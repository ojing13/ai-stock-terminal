        ...
        with tab1:
            ...
            if st.button("AI 차트 추세 분석 실행"):
                with st.spinner("순수 기술적 관점에서 차트를 분석하는 중입니다..."):
                    ...
                    3. [가독성 철저]: 각 추세 설명 시 반드시 소제목을 달고 줄바꿈(Enter)을 철저히 하여 읽기 편하게 작성하세요.
...
- [가독성 철저]: 1, 2, 3번 각 평가 항목마다 명확한 소제목을 달고, 세부 분석을 서술할 때는 반드시 줄바꿈(Enter)과 글머리 기호(-, *)를 적극 활용하여 텍스트가 뭉치지 않도록 하세요.
                ...
                with st.spinner('모든 데이터를 종합하여 리포트를 작성 중입니다...'):
                    try:
                        ...
                        4. 구체적인 가격 제시 (진입 추천가, 1차 목표가, 손절가)
                           - 각 가격을 제시할 때 반드시 줄바꿈(Enter)을 하세요.
                           - 그 아래에 '**논리적 근거:**'라는 소제목을 달고 다시 줄바꿈을 하여, 기술적 지표와 재무/동향을 융합한 구체적인 근거를 작성할 것.
                           (예시)
                           - 진입 추천가: 000 원
                             - **논리적 근거:** 이러이러한 지표를 고려할 때...
                        
                        🚨 [최고급 퀀트 애널리스트 수준의 입체적 분석 지침 - 반드시 엄수할 것]
                        - [가독성 철저]: '논리적 근거' 등의 소제목이나 새로운 항목이 시작될 때는 반드시 줄바꿈을 적용하여 글이 한 덩어리로 뭉치지 않게 하세요.
