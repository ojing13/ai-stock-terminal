    ...
    if not hist_basic.empty:
        ...
        with tab2:
            ...
            if st.button("AI 재무 건전성 평가 실행"):
                with st.spinner("방대한 재무 지표를 꼼꼼하게 분석 중입니다..."):
...
(🚨 주의: 마크다운 렌더링 오류를 막기 위해 절대 물결표(~) 및 달러 기호($)를 사용하지 마세요. 금액은 '{currency}'으로 표기하세요.)
"""
                    response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
                    st.info(response.text)
                ...
                with st.spinner('지표와 실시간 뉴스를 종합하여 리포트를 작성 중입니다...'):
                    try:
                        ...
                        4. 구체적인 가격 제시 (진입 추천가, 1차 목표가, 손절가)
                            - 이 세 가지 가격은 절대 임의로 산정하지 말고, 앞서 제공된 [기술적 지표], [재무지표], [실시간 뉴스] 이 세 가지 요소를 모두 융합하여 도출된 논리적 근거와 함께 구체적으로 제시해주세요.
                        """
                        response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
                        st.info(response.text)
