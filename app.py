        ...
        with tab4:
            ...
            if _cache_key in st.session_state.report_cache:
                ...
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    척도 기준점 (이 두 끝을 기준으로 이 종목의 절대적 위치를 잡으세요):
                    - 0에 가까울수록: 미국 단기국채(T-Bill) 수준 — 원금 손실 가능성이 사실상 없음
                    - 100에 가까울수록: FTX 거래소 상장 직전(2021년) 수준 — 사업모델 붕괴·사기·청산 위험이 현실적으로 존재했던 수준
                    이 종목은 그 사이 어디에 위치하는가를 냉정하게 판단하세요.
                    ...
                    척도 기준점 (이 두 끝을 기준으로 이 종목의 절대적 위치를 잡으세요):
                    - 0에 가까울수록: 미국 단기국채(T-Bill) 수준 — 이자 외 추가 상승이 구조적으로 없음
                    - 100에 가까울수록: 테슬라 2020년 초 수준 — 당시 주가에서 1년 내 10배 이상 상승이 실제로 발생한 수준
                    이 종목은 그 사이 어디에 위치하는가를 냉정하게 판단하세요.
                    테슬라 2020 수준의 상승 포텐셜은 극히 드뭅니다. 대부분의 종목은 자연스럽게 중하위권에 분포합니다.
