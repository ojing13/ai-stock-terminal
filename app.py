        ...
        with tab1:
            ...
            if st.button("AI 차트 추세 분석 실행"):
                with st.spinner("순수 기술적 관점에서 차트를 분석하는 중입니다..."):
                    ...
                    4. [핵심 강조]: 분석 내용 중 핵심이 되는 중요한 단어나 문장은 반드시 **굵은 글씨(**)**로 강조해서 한눈에 들어오게 하세요. 단, 폰트 크기나 색상은 절대 임의로 변경하지 마세요.
                    ...
                    핵심 요약 및 상세 분석 내용을 일반 문단으로 쭉 이어서 작성합니다. **주요 저항선이나 지지선 돌파**와 같이 중요한 내용은 이렇게 굵게 처리하세요. 글머리 기호 없이 빈 줄로 단락을 구분하세요.

                    ### 2. 장기적인 추세 (Long-term trend)

                    핵심 요약 및 상세 분석 내용을 일반 문단으로 작성합니다...
                    """
...
- [재무 지표 중심의 서술]: 제공된 텍스트 동향은 오직 '재무 지표의 원인과 결과' 파악하는 데만 조용히 참고하세요. 기술적 차트 이야기나 가십성 이슈는 철저히 배제하고, 철저히 '재무적 관점(수익성, 안정성, 현금흐름, 밸류에이션)'에만 집중해서 평가하세요.
