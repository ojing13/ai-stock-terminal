        ...
        with tab4:
            ...
            if _cache_key in st.session_state.report_cache:
                ...
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    점수를 정하기 전에 반드시 아래 두 질문을 먼저 검토하세요 (극단 배제 필터):
                    Q1. 이 종목이 미국 단기국채(T-Bill)보다 리스크가 낮거나 비슷한가?
                        → Yes라면 10 이하. 현금성 ETF·초단기채권 외엔 사실상 No.
                    Q2. 이 종목이 리먼브라더스 2008년 파산 직전 수준(과도한 레버리지·부실자산으로 실제 파산)에 가까운가?
                        → Yes라면 85 이상. 실제 파산·청산 위험이 가시적인 경우만 해당.
                    위 두 질문으로 범위를 좁힌 뒤, 그 안에서 이 종목의 고유한 리스크 수준을 정확히 표현하세요.

                    2. [RETURN: 0~100] — 지금 이 가격 기준, 이 종목의 장기 성장 잠재력과 추가 상승 포텐셜
                    ...
                    이 종목이 속한 산업과 사업 모델 자체의 미래 성장 가능성을 먼저 판단하세요.
                    - 산업 성장성: 이 종목이 속한 산업이 앞으로 몇 년간 얼마나 성장할 수 있는가? (AI, 양자컴퓨팅, 우주, 바이오 등 신산업 vs 전통 제조/유통 등 성숙 산업)
                    - 사업 모델 확장성: 신제품·신시장·신규 수익원으로 매출이 구조적으로 성장할 여지가 있는가?
                    - 경쟁 우위: 이 종목만의 해자(특허, 브랜드, 네트워크 효과, 독점적 기술)가 있어 장기 성장을 뒷받침하는가?
                    - 성장 단계: 초기 성장주(매출 고성장 중)인가, 성숙 대형주(성장 둔화)인가, 전환점에 있는 기업인가?

                    [B. 현재 가격 대비 업사이드 — 두 번째 축]
                    사업 성장 잠재력이 현재 주가에 얼마나 반영됐는가를 판단하세요.
                    - 밸류에이션 여유: 사업 성장성 대비 현재 주가가 저평가·적정·고평가 중 어디인가? (PEG, Forward PER, 업종 비교 등)
                    ...
                    척도 기준점(T-Bill ~ 테슬라 2020) 사이 어디에 이 종목이 위치하는지를 정직하게 표현하세요.
                    동일 업종이라도 기업의 경쟁 우위, 성장 단계, 시장 내 위치에 따라 점수가 크게 달라질 수 있습니다.

                    점수를 정하기 전에 반드시 아래 두 질문을 먼저 검토하세요 (극단 배제 필터):
                    Q1. 이 종목이 미국 단기국채(T-Bill)처럼 구조적으로 주가 상승이 없는 수준인가?
                        → Yes라면 10 이하. 현금성 ETF·초단기채권 외엔 사실상 No.
                    Q2. 이 종목이 아마존 2001~2020년 수준(닷컴 버블 저점 대비 약 200배, 순수 사업 성장의 극단적 케이스)에 가까운 잠재력인가?
                        → Yes라면 85 이상. 이 수준은 극히 드뭅니다. 웬만한 성장주는 해당하지 않습니다.
                    위 두 질문으로 범위를 좁힌 뒤, 그 안에서 이 종목의 고유한 성장 잠재력을 정확히 표현하세요.
                    ...
                    **판단근거:**
                    ...
                    - RETURN 근거: [산업·사업 성장 잠재력 + 현재 가격 기준 추가 상승 여력 판단 한 줄. 손실 가능성·하락 리스크는 절대 포함하지 말 것. 순수하게 "얼마나 더 오를 수 있는가"만 서술]
                    - SCORE 근거: [위의 RISK와 RETURN 수치를 바탕으로, 손익비(상승 여력 대비 손실 위험)를 종합한 최종 투자 판단 한 줄]
