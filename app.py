...
<style>
    ...
    * { font-family: 'Pretendard', 'Noto Sans KR', sans-serif !important; }
    ...
    .block-container { background-color: #f0f2f5 !important; padding-top: 2.5rem !important; }
    ...
    .ai-result-card {
        background-color: #e8eaed; border-radius: 14px; padding: 28px 32px; margin-top: 8px;
    }
    .ai-result-card p, .ai-result-card li { color: #374151; line-height: 1.9; font-size: 14.5px; }
    .ai-result-card h1, .ai-result-card h2, .ai-result-card h3 {
        color: #111827; font-weight: 800; font-size: 15.5px;
        margin-top: 28px; margin-bottom: 10px; padding-bottom: 8px;
        border-bottom: 1px solid #d1d5db; letter-spacing: -0.2px;
    }
    .ai-result-card strong { color: #111827; font-weight: 700; }
    ...
    [class*="actionButton"],
    button[kind="header"],
    #stDecoration { display: none !important; visibility: hidden !important; opacity: 0 !important; pointer-events: none !important; }
    ...
    [data-testid="stStatusWidget"]:not(:empty)::before {
        content: "";
        position: fixed;
        top: 14px; right: 18px;
    ...
    [data-testid="stStatusWidget"]:not(:empty)::after {
        content: "● ● ●";
        position: fixed;
        top: 14px; right: 18px;
        width: 90px; height: 30px;
...
def augment_us_fundamentals(ticker, info):
    ...
    try:
        ...
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', 'Accept': 'text/html', 'Upgrade-Insecure-Requests': '1'}
        res = requests.get(url, headers=headers, timeout=5)
        soup = BeautifulSoup(res.text, 'html.parser')
...
if user_input:
    ...
    if not hist_basic.empty:
        ...
        if ticker.endswith('.KS') or ticker.endswith('.KQ'):
            ...
            # 미국 주식: Yahoo Finance 검색 API → longName → shortName 순으로 정식 종목명 조회
            ...
            try:
                if not div_data.empty and current_p > 0:
                    ...
                    if not recent_divs.empty:
                        ...
                        if 0 < history_yield < 0.50: return history_yield
            except: pass
        ...
        with tab1:
            ...
            if not history.empty:
                ...
                if not filtered_history.empty:
                    xaxis_config = dict(rangeslider=dict(visible=False), type="date", hoverformat="%Y-%m-%d", fixedrange=True)
                    ...
                    fig.add_trace(go.Candlestick(
                        ...
                        increasing_line_color='#ff2d55', decreasing_line_color='#007bff', name="가격"
                    ))
                    for w, name, color in ma_settings:
                        fig.add_trace(go.Scatter(
                            x=filtered_history.index, y=filtered_history[f'MA_{w}'],
                    ...
                    fig.add_annotation(
                        ...
                        ax=0, ay=-35, font=dict(color="white", size=13, family="Pretendard"),
                    ...
                    fig.update_layout(
                        ...
                        height=520, margin=dict(l=0, r=10, t=48, b=0),
                        ...
                        hovermode="x unified", clickmode="none",
                    ...
                    st.plotly_chart(fig, use_container_width=True, config={
                        ...
                        'showAxisDragHandles': False, 'doubleClick': False
                    })
                else:
...
6. 핵심 강조: **분석에서 가장 중요하고 유의미한 문장 전체나 키워드**를 **굵은 글씨(**)**로 강조하세요.
7. 어조 설정: 정중체 사용. 깔끔한 전문가 톤 유지.
8. 항목 제한: 분석 항목은 무조건 '1. 단기적인 추세', '2. 장기적인 추세' 두 가지만 출력.
9. 출처 표기 절대 금지.
10. 뉴스 헤드라인 직접 인용 절대 금지.
"""
                    try:
...
[출력 형식 가이드]
- 각 항목의 제목은 마크다운 헤딩(## 또는 ###)을 사용하여 작성하세요.
- 3번 항목은 반드시 아래 형식을 따르세요:
  #### 현재 보유자
  [본문 서술]
  #### 신규 매수 대기자
  [본문 서술]
...
[분석 지침]
- 어조: 정중체 사용. 깔끔한 전문가 톤. 이모티콘 절대 사용 금지.
- 핵심 강조: 핵심 문장은 반드시 **굵은 글씨(**)**로 강조.
- 달러 기호 금지. 금액은 반드시 '{currency}'으로 표기.
- 출처 표기 절대 금지. 뉴스 헤드라인 직접 인용 절대 금지.
...
[전제] 자산의 20%를 이 종목에 투자할지 고민. 평생 보유 가정. 손실 허용 한도 -25%.
현재 주가 저평가/고평가 여부, 손익비 중심. 0(절대 안 한다) ~ 50(중립) ~ 100(강하게 확신).

2. [RISK: 0~100] — 종합적인 손실 위험도
재무/밸류에이션/사업구조/시장·거시 리스크 종합.
0(거의 무위험) ~ 100(매우 높은 손실 가능성).

3. [RETURN: 0~100] — 최대 상승 포텐셜
미래 시나리오가 실현됐을 때 주가 상승 잠재력. 베팅 가치 중심.
0(상승 기대 없음) ~ 100(수배 이상 폭발적 상승).

점수를 내기 전에 반드시 아래 순서로 근거를 먼저 서술하세요:
RISK 근거: 재무/밸류에이션/사업구조/거시 리스크 각각 한 줄씩
RETURN 근거: 상승 시나리오와 실현 가능성 한 줄
SCORE 근거: 손익비 판단 한 줄
