...
""", unsafe_allow_html=True)
st.markdown("""
<div style="font-size: 1.75rem; font-weight: 900; color: #1a1a2e; letter-spacing: -0.8px; line-height: 1.2; margin-bottom: 22px;">
...
def get_krx_candidates():
    """한국 흑자 저평가 후보 스크리닝"""
    try:
        kr_list = fdr.StockListing('KRX')
        if kr_list is None or kr_list.empty:
            return []
        candidates = []
...
def get_us_candidates():
    """미국 finviz 스크리닝: 흑자 + 저PER + 저PBR"""
    try:
        import urllib.request
        url = "https://finviz.com/screener.ashx?v=111&f=fa_eps_pos,fa_pe_u20,fa_pb_u2,sh_avgvol_o100&ft=4&o=marketcap"
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        html = urllib.request.urlopen(req, timeout=10).read().decode('utf-8')
        soup = BeautifulSoup(html, 'html.parser')
        rows = soup.select('tr.styled-row, tr[class*="row"]')
        candidates = []
        for row in rows:
            cols = row.find_all('td')
            if len(cols) < 12:
            ...
            try:
                ...
                mkt_cap_text = cols[6].get_text(strip=True)
                per_text = cols[9].get_text(strip=True)
                pbr_text = cols[10].get_text(strip=True)
                price_text = cols[8].get_text(strip=True)
                if not ticker_str or per_text in ['-', ''] or pbr_text in ['-', '']:
                    continue
                def parse_mktcap(s):
                    s = s.strip()
                ...
                candidates.append({
                    'ticker': ticker_str,
                    'name': name,
                    'market_cap': parse_mktcap(mkt_cap_text),
...
def score_candidate_quick(ticker_str, name, client_obj):
    """후보 종목 AI 스코어 빠르게 산출"""
    try:
        stk = yf.Ticker(ticker_str)
        info_d = stk.info
        ...
        low_52 = hist['Low'].min()
        pos_52 = round((cur_price - low_52) / (high_52 - low_52) * 100) if high_52 != low_52 else 50
        trailing_pe = info_d.get('trailingPE', 'N/A')
        forward_pe  = info_d.get('forwardPE', 'N/A')
        pb  = info_d.get('priceToBook', 'N/A')
        roe = info_d.get('returnOnEquity', 'N/A')
        op_margin = info_d.get('operatingMargins', 'N/A')
        mkt_cap = info_d.get('marketCap', None)
        currency = info_d.get('currency', 'KRW' if '.KS' in ticker_str or '.KQ' in ticker_str else 'USD')

        def fp(v): return f"{v*100:.1f}%" if isinstance(v, float) else str(v)
        def ff(v): return f"{v:.2f}" if isinstance(v, float) else str(v)

        prompt = f"""종목: {name} ({ticker_str})
52주 범위 위치: {pos_52}% (0%=최저, 100%=최고)
현재가: {cur_price:.2f} {currency} / 52주 고: {high_52:.2f} / 52주 저: {low_52:.2f}
Trailing PER: {ff(trailing_pe)}, Forward PER: {ff(forward_pe)}, PBR: {ff(pb)}
ROE: {fp(roe)}, 영업이익률: {fp(op_margin)}

[평가 목적]
이 종목이 "극도로 저위험 고수익"인지 판별하는 것이 목적입니다.
아래 기준을 엄격하게 적용하세요. 조건을 충족하지 못하면 점수를 낮게 주세요.

[RISK 낮음 조건 - 모두 충족해야 RISK 30 이하]
- 재무: 부채비율 낮고, 영업이익 흑자 안정적, 현금흐름 양호
- 밸류에이션: PER·PBR이 업종 평균 대비 명백히 저평가
- 사업: 경쟁우위 있고 사업 구조 안정적, 급격한 실적 악화 가능성 낮음
- 거시: 금리·환율·규제 리스크 낮음

[RETURN 높음 조건 - 모두 충족해야 RETURN 80 이상]
- 현재 저평가가 해소될 명확한 촉매(실적 개선, 업종 회복, 재평가 가능성)가 있음
- 52주 저점 부근이면 기술적 반등 포텐셜 추가 반영
- 장기 보유 시 복리 수익 기대 가능한 사업 구조

[SCORE 기준]
RISK ≤ 30 이고 RETURN ≥ 80 이면 → SCORE 70 이상 가능
그 외 조건 미달 시 → SCORE 50 이하로 엄격하게 평가
[투자자 전제: 자산 20%, 평생 보유, 손실 허용 -25%]

근거 서술 후 점수 출력:
RISK 근거: 한 줄 (위 조건 충족 여부 명시)
RETURN 근거: 한 줄 (위 조건 충족 여부 명시)
SCORE 근거: 한 줄
...
[RETURN: 숫자]"""
        ...
        txt = resp.text
        sm  = re.search(r'\[SCORE:\s*(\d+)\s*\]', txt)
        rm  = re.search(r'\[RISK:\s*(\d+)\s*\]', txt)
        rem = re.search(r'\[RETURN:\s*(\d+)\s*\]', txt)
        if not sm or not rm or not rem:
            return None
        return {
            'ticker': ticker_str,
            'name': name,
...
def pick_quota(scored, n_each=2):
    """시총 구간별 쿼터 선발 - 극도로 저위험고수익 기준"""
    # 극도로 저위험고수익 조건: RISK<=30 AND RETURN>=80
    def is_prime(x):
        return x['risk'] <= 30 and x['return_s'] >= 80

    def sort_key(x):
        nearness_bonus = max(0, (50 - x['pos_52']) * 0.3)
        prime_bonus = 30 if is_prime(x) else 0  # 조건 충족 종목 우선
        return (prime_bonus + x['return_s'] - x['risk'] + nearness_bonus)

    large  = [s for s in scored if s['market_cap'] and s['market_cap'] >= 1e12]
    mid    = [s for s in scored if s['market_cap'] and 1e11 <= s['market_cap'] < 1e12]
    small  = [s for s in scored if s['market_cap'] and s['market_cap'] < 1e11]

    result = []
    for group in [large, mid, small]:
        group.sort(key=sort_key, reverse=True)
        result.extend(group[:n_each])
...
def render_recommend(results, label):
    if not results:
        st.info(f"{label}: 조건 충족 추천 종목을 찾지 못했습니다.")
        return
                ...
                if kr_raw is not None and not kr_raw.empty:
                    for _, row in kr_raw.iterrows():
                        try:
                            ...
                            candidates_kr.append({'ticker': code + suffix, 'name': name_kr})
                        except:
                            continue
        ...
        with st.spinner("미국 시장 스크리닝 중... (30초~1분 소요)"):
            try:
                ...
                for _row in _soup.select('tr'):
                    ...
                    try:
                        ticker_str = cols[1].get_text(strip=True)
                        name_us = cols[2].get_text(strip=True)
                        ...
                        if per_v <= 0 or pbr_v <= 0:
                            continue
                        us_candidates.append({'ticker': ticker_str, 'name': name_us, 'per': per_v, 'pbr': pbr_v, 'market_cap': _mc(cap_t)})
                    except:
                        continue
        ...
        with tab4:
            ...
            if _cache_key in st.session_state.report_cache:
                ...
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    [전제] 자산의 20%를 이 종목에 투자할지 고민. 레버리지 상품이 아닌 이상 평생 팔지 않음.
