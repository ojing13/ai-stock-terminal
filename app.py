...
이 모든 세부 재무 수치들과 최신 동향을 종합적으로 분석하여 다음을 객관적으로 평가해주세요:
1. 현재 기업 가치의 고평가 또는 저평가 여부
2. 기업의 재무적 안전성 및 리스크 판단
3. 기업의 수익성 및 미래 성장 가능성

🚨 [최고급 퀀트 애널리스트 수준의 입체적 분석 지침 - 반드시 엄수할 것]
- [철저한 재무 중심 서술]: 이 리포트의 뼈대는 100% '재무 수치(수익성, 안정성, 현금흐름)'입니다. 산업의 일반적인 가십, 단순 주가 흐름, 신제품 홍보 등 재무 지표와 직접적인 관련이 없는 불필요한 설명은 철저히 배제하세요.
- [팩트를 통한 원인 규명]: 제공된 [최신 동향 데이터]는 오직 '특정 재무 지표가 왜 그렇게 변했는지' 원인을 짚어줄 때만 핀포인트로 꺼내 쓰세요. (예: "부채비율이 높은 이유는 최근 대규모 AI 데이터센터 인프라 확충에 따른 자금 조달의 결과이며, 이는 탄탄한 영업활동현금흐름으로 감당 가능한 수준입니다.") 동향을 설명하기 위해 지표를 쓰지 말고, 지표를 설명하기 위해 동향을 쓰세요.
- [출처 및 작위적 표현 절대 금지]: "뉴스에 따르면", "기사에서", "최신 동향을 보면", "표면적 지표 이면의", "숨겨진 리스크" 등 AI 프롬프트 지시어 느낌이나 출처를 암시하는 단어를 출력문에 일절(Never) 포함하지 마세요. 확신에 찬 전문가의 어조로 직접 분석한 팩트처럼 단정적으로 서술하세요.
- [입체적 재무 해석]: 부채비율이나 자본잠식을 맹목적으로 '주주환원에 의한 착한 부채'로 포장하지 마세요. 이자보상배율, 유동비율, 기업의 투자(CapEx) 규모를 교차 검증하여 이것이 감당 가능한 레버리지인지, 치명적인 리스크인지 오직 수치와 팩트를 근거로 냉철하게 판단하세요.
