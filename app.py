        ...
        with tab1:
            ...
            if st.button("AI 차트 추세 분석 실행"):
                with st.spinner("일봉, 주봉, 월봉 데이터를 모두 종합하여 순수 기술적 관점에서 차트를 분석하는 중입니다..."):
                    ...
                    [🚨 기술적 분석 핵심 지시사항 🚨]
                    1. [다중 주기 종합 분석 및 필터링]: 일봉, 주봉, 월봉 데이터를 모두 종합적으로 고려하여 큰 흐름과 단기 흐름을 유기적으로 파악하세요. 단, 모든 주기의 수치를 기계적으로 나열하지 말고 **설명에 꼭 필요한 핵심적이고 유의미한 정보(지표, 가격대 등)만 선별하여** 근거로 제시하세요. 불필요한 정보는 과감히 생략하세요.
