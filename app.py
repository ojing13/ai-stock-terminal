        ...
        with tab4:
            ...
            if st.button("원클릭 종합 분석 리포트 생성"):
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    동일한 데이터가 주어지면 항상 동일한 점수를 내도록 일관성을 유지하세요. 감정 배제, 객관적 판단.

                    점수 산출 원칙:
                    기계적 기준표 없이, 아래 요소들을 이 종목의 업종·성장단계·시장 맥락에 맞게 종합적으로 해석하여 점수를 부여하세요.
                    숫자 하나하나보다 전체 그림이 무엇을 말하는지를 판단하는 것이 핵심입니다.

                    1. [SCORE: 0~100] (AI 종합 투자의견)
                    다음 요소들을 이 종목에 맞게 종합 해석하세요:
                    - 현재 주가가 내재가치 대비 저평가인지 고평가인지 (PER, PBR, PSR, Forward PER, Trailing PER 괴리 등을 업종 특성에 맞게 해석)
                    - 현재 주가 자리의 손익비: 지금 진입 시 기대수익 대비 하락 리스크가 얼마나 유리한가
                    - 재무 건전성 (부채비율, 영업이익률, 현금흐름 등)
                    - 기업의 본질적 경쟁력과 미래 성장성 (AI 자체 판단)
                    - 매출·이익 성장 추세
                    → 0(강력매도) ~ 100(강력매수)으로 표현. 50이 중립.

                    2. [RISK: 0~100] (리스크 지수)
                    다음 요소들을 종합하여 현재 투자 시 감당해야 할 실질적 위험도를 판단하세요:
                    - 밸류에이션 부담: 현재 주가가 어느 정도 고평가되어 있는지
                    - 주가 위치: 고점 대비 위치, 추가 하락 여지
                    - 재무 리스크: 부채, 유동성, 이익 감소 추세
                    - 기업·산업 구조적 리스크 (AI 자체 판단)
                    → 0(매우 안전) ~ 100(매우 위험)

                    3. [RETURN: 0~100] (기대수익 지수)
                    다음 요소들을 종합하여 현재 시점의 상승 잠재력을 판단하세요:
                    - 저평가 매력: 내재가치 대비 현재 주가의 할인율 (업종 맥락 고려)
                    - 현재가의 손익비 우위: 지지선·이평선·역사적 저점 등과의 관계에서 반등 여력
                    - 이익 성장 기대: Forward 실적 개선 폭, 매출 성장률
                    - 기업 미래 성장 동력 (AI 자체 판단)
                    → 0(수익 기대 없음) ~ 100(강한 상승 잠재력)
                    """
                    try:
                        response = client.models.generate_content(
                            model='gemini-2.5-flash', contents=prompt, config={"temperature": 0.0}
