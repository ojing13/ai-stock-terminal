        ...
        with tab2:
            ...
            if st.button("AI 재무 건전성 평가 실행"):
                with st.spinner("재무적 지표를 바탕으로 입체적인 분석을 진행 중입니다..."):
                    prompt = f"""종목 {ticker}의 상세 재무 데이터 및 최근 핵심 뉴스 15개 동향입니다.
...
2. 기업의 재무적 안전성 (표면적 지표 이면의 진짜 리스크 여부 판단)
3. 기업의 수익성 및 미래 성장 가능성

🚨 [최고급 애널리스트 수준의 입체적 분석 지침 - 반드시 엄수할 것]
- [배경 지식 통합]: 제공된 뉴스 헤드라인뿐만 아니라, 당신이 학습한 해당 기업의 최근 거시경제 환경, 산업 트렌드, 대규모 투자(CapEx) 현황 등을 총동원하여 수치의 인과관계를 역추적하세요.
- [입체적 재무 해석]: 부채비율이 높거나 자본잠식 상태일 때, 이를 무조건 '착한 부채'로 포장하지 마세요. '이자보상배율', '영업활동현금흐름', 그리고 '최신 뉴스'를 교차 검증하여 현재의 부채가 성장을 위한 건전한 레버리지인지, 아니면 과도한 인프라/인수합병 투자로 인해 실제로 시장이 우려하는 이자 비용 등의 부담인지 냉철하게 판단하세요.
- [시장 우려 및 리스크 직시]: 특정 지표가 악화되었거나 부정적 뉴스가 있다면, 이것이 실적 둔화로 이어질지 혹은 일시적인 노이즈인지 낱낱이 파헤치세요. 무조건적인 장밋빛 전망은 배제하세요.
- [기계적 해석 금지]: PBR, PER 등의 수치가 비정상적(음수, 과도한 고평가 등)일 때 교과서적인 잣대를 기계적으로 들이대지 마세요.
- [뉴스 직접 인용 절대 금지]: "최신 뉴스에 따르면~", "기사 제목에서 알 수 있듯이~" 같은 촌스러운 표현을 절대 쓰지 마세요. 마치 당신이 처음부터 시장 상황을 꿰뚫어 보고 있었던 애널리스트처럼 자연스럽게 서술하세요.
            ...
            if st.button("원클릭 종합 분석 리포트 생성"):
                with st.spinner('지표와 실시간 뉴스를 종합하여 리포트를 작성 중입니다...'):
                    try:
                        ...
                        1. 재무 상황 종합 평가 (숫자 이면의 진짜 리스크와 기회 도출)
                        2. 시장 투심 및 향후 주가 흐름 예상 (Fear & Greed, 매크로 동향 반영)
                        ...
                        - [배경 지식 총동원]: 제공된 수치와 뉴스에만 갇히지 마세요. 당신이 학습한 해당 기업의 최근 거시경제(금리, 인플레 등) 환경, 산업 트렌드(AI, 반도체 등), 경쟁사 동향, 대규모 투자(CapEx) 현황을 융합하여 인과관계를 셜록 홈즈처럼 역추적하세요.
                        - [맹목적 긍정 금지 및 리스크 직시]: 부채비율이 높거나 자본잠식 상태일 때, 무조건 주주환원(자사주 매입)에 의한 '착한 부채'로 포장하지 마세요. '이자보상배율', '현금흐름', '뉴스'를 교차 검증하여, 과도한 인프라/M&A 투자로 인한 이자 부담이나 시장이 실제로 우려하는 치명적 리스크라면 아주 냉철하게 경고하세요.
                        - [시장 심리(Fear & Greed) 통찰]: 주가가 크게 하락했거나 변동성이 크다면, 뉴스 행간의 의미를 파악해 현재 시장 참여자들이 무엇에 공포를 느끼고 있는지, 혹은 어떤 과도한 탐욕이 껴있는지 평가에 반드시 명확히 반영하세요.
                        - [기계적 해석 금지]: PBR, PER, PEG 등의 수치가 비정상적(음수, 과도한 고평가 등)일 때 교과서적인 잣대를 들이대지 마세요.
                        - [뉴스 직접 인용 절대 금지]: "최신 뉴스에 따르면", "기사 제목에서 알 수 있듯이" 같은 촌스러운 표현을 절대 쓰지 마세요. 처음부터 시장 상황을 꿰뚫어 보고 있었던 것처럼 자연스럽게 펀더멘털 분석에 녹여내세요.
                        - 마크다운 렌더링 오류를 막기 위해 절대 물결표(~) 및 달러 기호($)를 사용하지 마세요. (금액은 반드시 '{currency}'으로 표기할 것)
                        """
                        response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
                        st.info(response.text)
