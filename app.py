        ...
        with tab4:
            ...
            if _cache_key in st.session_state.report_cache:
                ...
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    [판단 전제]
                    자신이 보유한 전체 자산의 20%를 이 종목 하나에 투자할지 고민하는 투자자를 가정하세요.
                    - 보유 기간: 레버리지 상품이 아닌 이상 평생 팔지 않는다. 단기 등락은 신경 쓰지 않음.
                    - 손실 허용 한도: 최대 -25% (기업 가치가 영구적으로 -25% 이상 훼손될 가능성이 높다면 투자하지 않는다)
                    - 합리적이고 냉정한 판단을 내리는 투자자. 과도한 낙관도 비관도 없음.
                    이 투자자의 관점에서 아래 세 점수를 평가하세요.
                    ...
                    1. [SCORE: 0~100] — 종합 투자의견: 이 종목에 자산의 20%를 평생 보유할 각오로 넣겠는가?
                    RISK와 RETURN을 모두 고려한 최종 판단. 아래 두 가지를 반드시 입체적으로 고려하세요:
                    (a) 현재 주가가 저평가인가 고평가인가: PER, PBR, Forward PER, 역사적 밸류에이션 범위 등을 업종 맥락에 맞게 해석.
                    (b) 손익비: 기업 가치가 영구적으로 -25% 이상 훼손될 가능성 대비, 장기적으로 기업 가치가 크게 성장할 수 있는가.
                    "이 기업을 평생 보유할 만큼 사업 구조와 장기 성장성이 믿을 만한가"가 핵심.
                    0(절대 안 한다) ~ 50(중립) ~ 100(강하게 확신하고 넣는다).

                    2. [RISK: 0~100] — 기업 가치가 영구적으로 -25% 이상 훼손될 위험도
                    단기 변동성이 아닌, 장기적으로 기업 가치 자체가 훼손될 가능성에 집중:
                    - 재무 리스크: 부채 수준, 유동성, 이익 감소 추세, 현금흐름 악화, 존속 가능성
                    - 밸류에이션 리스크: 현재 주가 고평가 여부(PER·PBR 업종 맥락 반영)
                    - 사업 구조 리스크: 사업 실패 가능성, 경쟁 심화, 산업 구조적 쇠퇴
                    - 시장/거시 리스크: 금리, 환율, 규제, 지정학 등 외부 요인
                    업종 특성 필수 반영(금융주 고부채=정상, 바이오 적자=감안 등).
                    0(거의 무위험) ~ 100(기업 가치 영구 훼손 가능성 매우 높음).

                    3. [RETURN: 0~100] — 장기 보유 시 기업 가치 성장 가능성
                    현재 재무 상황이 아니라, 이 기업이 장기적으로 얼마나 성장할 수 있는가.
                    미래 시나리오(신사업 성공, 대형 계약, 시장 독점, 산업 전환 수혜 등)의 실현 가능성과 규모를 평가.
                    현재 주가가 저평가 구간이면 그 자체로 장기 수익 여력이 높으므로 반영.
                    0(성장 기대 없음) ~ 50(완만한 성장 가능) ~ 100(폭발적 장기 성장 강하게 확신).

                    중요: 세 점수가 서로 논리적으로 일관되어야 합니다.
                    (RISK=80 → 영구 손실 가능성 높음 → SCORE≤40 / RETURN=20 → 장기 성장 기대 없음 → SCORE 높기 어려움)
                    1단위로 정밀하게 평가하세요.
