        ...
        with tab4:
            ...
            if _cache_key in st.session_state.report_cache:
                ...
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    점수 산출 전 아래 두 질문에 먼저 스스로 답하세요. 답은 출력하지 않습니다.

                    [RISK 내부 판단 - 출력 금지]
                    Q1. 부채비율·유동비율·현금흐름 기준으로 단기 생존 가능성은?
                    Q2. 52주 위치와 PEG(없으면 PER) 기준으로 현재 주가가 고평가되어 추가 하락 여지가 있는가?
                    ...
                    **판단근거:** (위 내부 판단을 마친 뒤 아래를 출력하세요)
                    - RISK 근거: [위 RISK 내부 판단을 한 문장으로 요약. 업종 특성 반영. 30자 이상 80자 이내]
                    - RETURN 근거: [위 RETURN 내부 판단을 한 문장으로 요약. 손실·하락 위험 언급 금지. 30자 이상 80자 이내]
