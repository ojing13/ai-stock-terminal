        ...
        with tab4:
            ...
            if _cache_key in st.session_state.report_cache:
                ...
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    반드시 다음 5가지 항목을 포함하여 한국어로 명확하게 작성해주세요.
                    
                    1. 재무 상황 종합 평가
                    2. 현재 시장 투심 현황 (뉴스와 시장 분위기를 중립적으로 브리핑만. 판단 금지)
                    3. 향후 주가 흐름 예상 (재무 지표·기술적 분석에만 근거. 뉴스 논조·투심·애널리스트 의견은 근거로 삼지 말 것)
                    4. 상황별 대응 전략 (현재 보유자 / 신규 매수 대기자 / 매도 고려자)
                    5. 구체적인 가격 제시 (진입 추천가, 1차 목표가, 손절가)
                    ...
                    - 4번 항목(상황별 대응 전략)은 반드시 아래 형식을 그대로 따르세요:
                      #### 현재 보유자
                      [본문 서술]
                    ...
                    - 5번 항목(구체적인 가격 제시)은 반드시 아래 형식을 그대로 따르세요. "(설명)" 같은 라벨 절대 금지:
                      #### 진입 추천가: [가격 또는 가격범위]
                    ...
                    - 뉴스 헤드라인 직접 인용 절대 금지: 기사 제목이나 헤드라인을 따옴표로 그대로 쓰지 마세요.
                    - 애널리스트 투자의견·목표주가·매수매도 추천은 이해충돌 가능성이 있으므로 주가 흐름 판단에 반영하지 마세요. 실적·계약·제품 출시·규제 등 객관적 사실만 근거로 삼으세요.
                    ...
                    **판단근거:**
                    ...
                    뉴스의 논조나 시장 분위기에 휩쓸리지 마세요. 뉴스는 사실(실적·계약·제품·규제)만 참고하고, 애널리스트 투자의견·목표주가·매수매도 추천은 이해충돌 가능성이 있으므로 무시하세요.
