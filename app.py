        ...
        with tab4:
            ...
            if st.button("원클릭 종합 분석 리포트 생성"):
                with st.spinner('모든 데이터를 종합하여 분석하는 중입니다...'):
                    ...
                    그 전제 위에서, 지금까지 분석한 모든 정보(재무, 밸류에이션, 주가 위치, 손익비, 기업 경쟁력, 성장성 등)를 
                    당신 스스로의 판단으로 종합하여 아래 세 점수를 매기세요. 어떤 기준표도 없습니다. 당신이 실제로 투자자라면 어떻게 느끼는지가 기준입니다.

                    1. [SCORE: 0~100] — 이 종목에 지금 500만원을 넣고 싶은가?
                    0에 가까울수록 "절대 안 한다", 100에 가까울수록 "강하게 확신하고 넣는다". 50은 중립.

                    2. [RISK: 0~100] — 이 투자에서 손실이 날 위험이 얼마나 되는가?
                    0에 가까울수록 매우 안전, 100에 가까울수록 매우 위험.

                    3. [RETURN: 0~100] — 이 종목의 중장기 수익 잠재력은 얼마나 되는가?
                    0에 가까울수록 기대 없음, 100에 가까울수록 강한 상승 잠재력.
                    """
                    try:
                        response = client.models.generate_content(
                            model='gemini-2.5-flash', contents=prompt, config={"temperature": 0.0}
